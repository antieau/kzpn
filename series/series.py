...
class WeightedPowerSeriesRingCappedElement:
    ...
    def __init__(self, parent, term_list):
        ...
        for term in term_list:
            if term[0] != term[1].degree():
                raise TypeError(
                    "Input 'term_list' must be a list of pairs (N,g) where n is a non-negative integer and g is a homogeneous polynomial of degree N."
                )
    ...
    def __neg__(self):
        return self.__class__(
            self.parent(), [(deg, -coeff) for deg, coeff in self._term_list]
        )
    ...
    def is_unit(self):
        if (
            self._term_list[0][0] != 0
            or not self._term_list[0][1].constant_coefficient().is_unit()
        ):
            return False
        else:
            return True
            ...
            [
                (
                    ...
                    self.parent()._polynomial_ring(
                        self._term_list[0][1].constant_coefficient().inverse()
                    ),
                )
            ],
        )
    ...
    def __rmul__(self, other):
        if other.parent() != self.parent().coefficient_ring():
            ...
            return self.__class__(
                self.parent(),
                [(deg, other * coefficient) for deg, coefficient in self._term_list],
            )
